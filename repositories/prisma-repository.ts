...
export function createPrismaRepositories(client: Client = defaultPrisma): RepositoryBundle {
  return {
    ...
    research: {
      async create(input: ResearchCreateInput) { const row = await client.researchResult.create({ data: { productId: input.productId, taskId: input.taskId, competitorCount: input.competitorCount, coverage: input.coverage, priceRange: input.priceRange, opportunities: json(input.opportunities), source: input.source } }); return toResearch(row); },
...
function toResearch(row: { id: string; productId: string; taskId: string | null; competitorCount: number; coverage: string; priceRange: string; opportunities: Prisma.JsonValue; source: string; createdAt: Date }): ResearchResultRecord { return { ...row, opportunities: Array.isArray(row.opportunities) ? row.opportunities.map(String) : [], createdAt: row.createdAt.toISOString() }; }
