...
export function createInMemoryRepositories(seed = false): RepositoryBundle {
  ...
  const tasksRepo: TaskRepository = {
    ...
    async listByStatus(status) { return taskRows.filter(row => row.status === status); },
  };
  const listingsRepo = { async create(input: ListingCreateInput) { const versions = listingRows.filter(row => row.productId === input.productId); const timestamp = now(); const row: ListingRecord = { id: id("listing"), productId: input.productId, taskId: input.taskId ?? null, version: versions.length + 1, title: input.title, titleZh: input.titleZh ?? null, bulletsZh: input.bulletsZh ?? [], descriptionZh: input.descriptionZh ?? null, researchContext: input.researchContext ? structuredClone(input.researchContext) : null, bullets: input.bullets, description: input.description ?? null, keywords: input.keywords, status: input.status ?? "draft", aiUsage: input.aiUsage ?? null, createdAt: timestamp, updatedAt: timestamp }; listingRows.push(row); return row; }, async latestByProduct(productId: string) { return [...listingRows].filter(row => row.productId === productId).sort((a, b) => b.version - a.version)[0] ?? null; }, async listByProduct(productId: string) { return listingRows.filter(row => row.productId === productId).sort((a, b) => b.version - a.version); } };
  const researchRepo = { async create(input: ResearchCreateInput) { const row: ResearchResultRecord = { ...input, competitorSnapshot: structuredClone(input.competitorSnapshot ?? []), id: id("research"), createdAt: now() }; researchRows.push(row); return row; }, async latestByProduct(productId: string) { return [...researchRows].filter(row => row.productId === productId).sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0] ?? null; } };
